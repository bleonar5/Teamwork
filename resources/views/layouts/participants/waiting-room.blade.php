...
$( document ).ready(function() {
  ...
  channel.bind('alert-waiter', function(data) {
    ...
    if(userId === data['user']['id']){
      ...
      $('#time_remaining').text(parseInt('{{ env("SESSION_LENGTH",300) }}') + parseInt('{{ env("SURVEY_LENGTH",120) }}') + parseInt('{{ env("BUFFER_LENGTH",30) }}') > 0 ? new Date(parseInt('{{ env("SESSION_LENGTH",300) }}') + parseInt('{{ env("SURVEY_LENGTH",120) }}') + parseInt('{{ env("BUFFER_LENGTH",30) }}') * 1000).toISOString().substr(14, 5) : 'N/A');
