...
#HOLDS FUNCTIONS FOR MANAGING GROUP TASKS

class GroupTaskController extends Controller
{
  //GETS NEXT TASK ON TASK LIST AND DISPLAYS
  public function getTask(Request $request) {

    //MARK USER ACTIVE SINCE THEY ARE JOINING A TASK
    ...
    // If there are no tasks at all, let's create some
      ...
      $groupTasks = GroupTask::initializeCryptoPilotTasks($group_id, $randomize = true);
    }
  ...
  //ROUTES USER TO THEIR TASK
  public function routeTask($task) {
    //SAVE PROGRESS
    $this->getProgress();

    //DISPLAYS APPROPRIATE TASK
    switch($task->name) {
      case "Intro":
  ...
  //RECORDS INFO FOR TASK AND PROCEEDS
  public function endTask(Request $request) {

    //USER NO LONGER ACTIVE FOR THE MOMENT
    $user = User::find(\Auth::user()->id);
    $user->status = 'Inactive';
    $user->save();

    //GET CURRENT TASK WITH RESPONSES
    ...
    //NOW THAT USER IS MARKED DONE, CHECK IF WHOLE GROUP IS DONE
    $task = \Teamwork\GroupTask::with('response')
      ->with('progress')
      ->find($request->session()->get('currentGroupTask'));
    ...
    //IF WHOLE GROUP IS DONE, PROCEED
    ...
    //OTHERWISE, DISPLAY WAIT SCREEN UNTIL GROUPMATES ARE FINISHED
    else {
      return redirect('/waiting');
    }
  }

  //grabs name of task (for memory)
  ...
  //PROCESSES STILL-PRESENT PINGS AND MARKS INACTIVE IF PINGS AREN'T FOUND
  public function stillPresent(Request $request) {
    $user_id = \Auth::user()->id;
    $this_user = User::where('id',$user_id)->first();

    //IF USER WAS INACTIVE BEFORE, MARK ACTIVE
    if($this_user->status){
    ...
    //UPDATE DB TIMESTAMP SO WE KNOW HOW LONG IT'S BEEN
    $this_user->touch();

    //CHECK IF OTHER USERS IN GROUP HAVE BEEN MISSING FOR MORE THAN 10 SECONDS
  ...
  public function memory(Request $request) {
    ...
    //MEMORY TASK SPANS MULTIPLE SUBTYPES
    ...
    //GET USER SCORES FROM INDIVIDUAL TO DETERMINE ROLES
  ...
  //SEND LEADER ANSWERED EVENT FOR MEMORY TASK
